...
const localCard=x=>'<article class="u-card">'+(x.verified===false?'<span class="badge watch">NÊN GỌI TRƯỚC</span>':'')+'<strong>'+esc(x.label)+'</strong><a class="phone" href="'+phoneHref(x.phone)+'">'+esc(x.phone)+'</a>'+(x.phone_alt?'<a class="phone" style="font-size:16px;margin-left:9px" href="'+phoneHref(x.phone_alt)+'">'+esc(x.phone_alt)+'</a>':'')+'<small>'+esc(x.note)+'</small><a class="source" href="'+esc(x.source)+'" target="_blank" rel="noopener">Trang chính thức →</a></article>';
...
const dirCard=x=>'<article class="u-card">'+(x.verified===false?'<span class="badge watch">NÊN GỌI TRƯỚC</span>':'')+'<strong>'+esc(x.label)+'</strong>'+(x.phone?'<a class="phone" href="'+phoneHref(x.phone)+'">'+esc(x.phone)+'</a>':'')+(x.phone_alt?'<a class="phone phone-alt" href="'+phoneHref(x.phone_alt)+'">'+esc(x.phone_alt)+'</a>':'')+'<small>'+esc(x.note)+'</small>'+((x.app_url||x.whatsapp||x.zalo)?'<div class="dir-contact-actions">'+(x.app_url?'<a href="'+esc(x.app_url)+'" target="_blank" rel="noopener">Mở app / website ↗</a>':'')+(x.whatsapp?'<a href="'+esc(x.whatsapp)+'" target="_blank" rel="noopener">WhatsApp ↗</a>':'')+(x.zalo?'<a href="'+esc(x.zalo)+'" target="_blank" rel="noopener">Zalo ↗</a>':'')+'</div>':'')+'<a class="source" href="'+esc(x.source)+'" target="_blank" rel="noopener">Xem trang gốc →</a></article>';
