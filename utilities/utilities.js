...
const dirCard=x=>'<article class="u-card"><span class="badge '+(x.verified===false?'watch':'')+'">'+(x.verified===false?'CẦN KIỂM TRA':'ĐÃ ĐỐI CHIẾU')+'</span><strong>'+esc(x.label)+'</strong><a class="phone" href="'+phoneHref(x.phone)+'">'+esc(x.phone)+'</a><small>'+esc(x.note)+'</small>'+((x.whatsapp||x.zalo)?'<div class="dir-contact-actions">'+(x.whatsapp?'<a href="'+esc(x.whatsapp)+'" target="_blank" rel="noopener">WhatsApp ↗</a>':'')+(x.zalo?'<a href="'+esc(x.zalo)+'" target="_blank" rel="noopener">Zalo ↗</a>':'')+'</div>':'')+'<a class="source" href="'+esc(x.source)+'" target="_blank" rel="noopener">Xem trang chính thức →</a></article>';
