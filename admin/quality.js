...
function renderQualityTasks(tasks,canManage=false){
  ...
  return tasks.slice().sort((a,b)=>(rank[a.severity]??3)-(rank[b.severity]??3)||String(a.surface||"").localeCompare(String(b.surface||""),"vi")).map(task=>{
    ...
    const controls=canManage?'<div class="task-controls" data-task-key="'+esc(key)+'">'+
      (task.status==="resolved"||task.status==="muted"
        ?'<button type="button" data-quality-action="reopen">Mở lại</button>'
        :'<button type="button" data-quality-action="claim">Nhận việc</button>'+
         '<button type="button" data-quality-action="refresh">Kiểm tra lại</button>'+
         (task.severity==="high"?"":'<button type="button" data-quality-action="mute">Ẩn 7 ngày</button>'))+
      '<label class="task-due-label">Hạn xử lý<input type="date" data-quality-due value="'+esc(String(task.due_at||"").slice(0,10))+'"></label><button type="button" data-quality-action="due">Lưu hạn</button></div>':"";
