...
const state={data:null,route:"all",operator:"all",mode:"all"};
...
function availability(r){
  ...
  return["Chưa có live inventory","info"];
}
...
$("#routeFilter").onchange=e=>{state.route=e.target.value;render()};$("#operatorFilter").onchange=e=>{state.operator=e.target.value;render()};$$("[data-mode]").forEach(b=>b.onclick=()=>{state.mode=b.dataset.mode;$$("[data-mode]").forEach(x=>x.classList.toggle("active",x===b));render()});$("#refreshBtn").onclick=load;load();