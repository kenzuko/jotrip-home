...
async function load(){try{$("#refreshBtn").textContent="…";const r=await fetch(DATA+"?t="+Date.now(),{cache:"no-store"});if(!r.ok)throw new Error(r.status);state.data=await r.json();fillFilters(state.data.departures.filter(x=>x.type==="sea"));render()}catch(e){$("#rows").innerHTML='<tr><td colspan="8" class="empty">Chưa tải được dữ liệu tàu và phà. Vui lòng kiểm tra trực tiếp với hãng trước khi ra bến.</td></tr>';$("#health").textContent="Chưa có dữ liệu";$("#updatedAt").textContent="Không kết luận tình trạng vận hành khi nguồn chưa phản hồi"}finally{$("#refreshBtn").textContent="↻"}}
