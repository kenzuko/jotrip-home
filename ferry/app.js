...
function render(){
  ...
  $("#health").textContent=!sourceGood?"Chưa lấy đủ lịch":fresh?"Lịch còn mới":"Nên xem lại";
  ...
  $("#rows").innerHTML=rows.length?rows.map(r=>{const [av,cls]=availability(r);return'<tr><td><strong>'+time(r.departure_time)+'</strong>'+(r.arrival_time?'<small>Đến '+time(r.arrival_time)+'</small>':'')+'</td><td>'+typeLabel(r)+'</td><td><strong>'+esc(r.operator)+'</strong></td><td>'+esc(r.origin)+' → '+esc(r.destination)+'</td><td>'+esc(r.vessel_or_service||"-")+'</td><td><span class="pill '+(/ĐÃ XUẤT BẾN/i.test(r.status||"")?"info":/MỞ BÁN/i.test(r.status||"")?"good":"watch")+'">'+esc(r.status||"-")+'</span></td><td><span class="pill '+cls+'">'+esc(av)+'</span></td><td><a href="'+esc(r.source_url||"#")+'" target="_blank" rel="noopener">'+esc(r.data_kind==="operational_public"?"Tình hình hôm nay":"Lịch hãng")+' ↗</a></td></tr>'}).join(""):'<tr><td colspan="8" class="empty">Ngày này chưa có lịch để hiện ở đây. Bạn có thể mở thẳng trang hãng bên dưới để xem.</td></tr>';
}
