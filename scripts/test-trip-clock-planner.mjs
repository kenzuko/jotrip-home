...
const afternoon=plan.select(rows(14),8);
assert.equal(afternoon.visible.length,8);
assert.equal(afternoon.total,13);
assert.equal(afternoon.hidden.length,5);
assert.equal(plan.select(rows(14)).visible.length,8);
...
const morning=plan.select(rows(8),8);
...
const rushBoard=plan.select([...rush,get(14,0,kiss)],8);
assert.equal(rushBoard.visible.length,8);
assert.ok(!rushBoard.visible.some(x=>x.item.entity_id===kiss),"Never displace ten near-term high-level activities");
const tuesday=plan.select(rows(14,0,"unknown",2),8);
...
assert.ok(plan.select(rows(21,5),8).visible.length<=8);
