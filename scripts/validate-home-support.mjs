...
const ids=new Set();
for(const file of fs.readdirSync(path.join(root,"data","entities")).filter(x=>x.endsWith(".json"))){const p=JSON.parse(fs.readFileSync(path.join(root,"data","entities",file),"utf8"));for(const e of p.entities||[])if(e.id)ids.add(e.id)}
