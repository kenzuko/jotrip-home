...
for(const item of support.near_me?.items||[]){const entity=entityMap.get(item.utility_id);if(!entity)errors.push("near_me references missing utility: "+item.utility_id);else{if(entity.entity_type!=="utility")errors.push(item.utility_id+": near_me target must be utility");if(entity.utility_type!==item.utility_type)errors.push(item.utility_id+": utility_type mismatch");if(!entity.address)errors.push(item.utility_id+": near_me utility missing address");if(entity.gps_eligible===true&&(!Number.isFinite(entity.map?.lat)||!Number.isFinite(entity.map?.lon)))errors.push(item.utility_id+": gps_eligible requires verified coordinates");}}
