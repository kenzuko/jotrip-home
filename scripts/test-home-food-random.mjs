...
assert.match(nodes.foodNowGrid.innerHTML,/Khám phá món này/);
assert.doesNotMatch(nodes.foodNowGrid.innerHTML,/Đang chọn món/);
const first=nodes.foodNowGrid.innerHTML.match(/food\/article\.html\?id=([^"]+)/)?.[1];
assert.ok(first,"a real dish detail URL must render");
...
const next=nodes.foodNowGrid.innerHTML.match(/food\/article\.html\?id=([^"]+)/)?.[1];
assert.ok(next,"randomized dish should also link to its detail");
assert.notEqual(first,next,"tapping random should change the dish when options exist");
console.log("Homepage food: one in-place randomized dish, working button and valid JS PASS");
