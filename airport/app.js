...
function renderSummary(){const l=state.latest;$('#totalFlights').textContent=l.counts?.total??'-';$('#arrivalsCount').textContent=l.counts?.arrivals??'-';$('#departuresCount').textContent=l.counts?.departures??'-';$('#internationalCount').textContent=l.summary?.arrivals_market?.international??'-';const age=ageInfo(l.collected_at_vn);const sourceText=state.dataSource==='live'?'Open AutoSync':state.dataSource==='fallback'?'AutoSync fallback':'Open AutoSync';$('#updatedAt').textContent=`Cập nhật ${new Date(l.collected_at_vn).toLocaleTimeString('vi-VN',{hour:'2-digit',minute:'2-digit',timeZone:'Asia/Ho_Chi_Minh'})} · ${age.label} trước · ${sourceText} · tự làm mới 1 phút`;const sl=$('.source-label');if(sl)sl.textContent=state.dataSource==='live'?'nguồn chính thức · Open AutoSync':'nguồn chính thức · Open AutoSync';const abnormal=(l.records||[]).filter(r=>!isPastRecord(r)&&changed(r)).length;const ops=$('#opsState');ops.className='ops-state';if(age.level==='stale'||age.level==='bad'){ops.textContent='DATA STALE';ops.classList.add('bad')}else if(abnormal>0||age.level==='watch'){ops.textContent='WATCH · '+abnormal+' BẤT THƯỜNG';ops.classList.add('watch')}else ops.textContent='OPERATIONS NORMAL'}
function renderHealth(){const l=state.latest,h=state.health,age=ageInfo(l.collected_at_vn);const qa=!!(h?.collector_completed&&h?.parser_passed&&h?.normalization_passed&&h?.qa_passed&&l?.quality?.usable);$('#qaText').textContent=qa?'PASS':'FAIL';$('#qaBadge').textContent=qa?'QA PASS':'QA FAIL';$('#qaBadge').className='qa-badge '+(qa?'good':'bad');$('#dataAge').textContent=age.label;let title,desc,level=age.level;if(!qa){level='bad';title='QA FAILED';desc='Nguồn hoặc normalization chưa đạt. Không nên dùng số liệu để điều hành.'}else if(age.level==='good'){title='GOOD · dữ liệu đang mới';desc=state.dataSource==='live'?'Open AutoSync đang đọc trực tiếp API chính thức nguồn chính thức, có cache ngắn để giữ ổn định.':'Snapshot Open AutoSync đã qua normalization và QA.'}else if(age.level==='watch'){title='WATCH · dữ liệu chậm cập nhật';desc=state.dataSource==='live'?'Nguồn live đang có độ trễ cao bất thường. Trang tự kiểm tra lại mỗi 1 phút.':'Snapshot vẫn hợp lệ nhưng đã quá 8 phút.'}else{title='STALE · không còn là dữ liệu live';desc='Dữ liệu đã quá 15 phút. Không nên xem đây là trạng thái tức thời của sân bay.'}if(state.dataSource==='fallback'&&qa){level=age.level==='stale'?'stale':'watch';title='FALLBACK · đang dùng AutoSync';desc='Open AutoSync tạm không phản hồi. Giao diện đã tự chuyển sang snapshot dự phòng gần nhất.'}setHealth(level,title,desc)}
