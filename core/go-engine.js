...
(function(root,factory){
  ...
  function fit(e,cfg,input,drive,clockValue,budget){
    ...
    const opts=windows(e,cfg,clockValue).map(w=>{
      if(w.fixed){
        if(arrival>w.start||(w.start+w.end-w.start)>limit)return null;
        const finish=w.end;
        return {start:w.start,finish,arrival,deadline:w.start-(cfg.entry_buffer_min||10)-drive.minutes,window:w,margin:w.start-arrival};
      }
  ...
  function assessLive(e,cfg,live,notices,day){
    ...
    if(outdoor){
      if(!weather.source_updated_at||["stale","unknown"].includes(weather.freshness||"unknown")||
         !["fresh","aging"].includes(weather.freshness))
  ...
  function plan(input){
    ...
    for(const cfg of cfgList){
      ...
      const notice=assessLive(e,cfg,input.live,input.notices,when.day);
      if(notice.blocked){excluded.push({id:e.id,reason:notice.reason});continue}
      ...
      results.push({
        ...
        image_id:cfg.image_id||e.id,category:cfg.category||"TRẢI NGHIỆM",
        badge,travel:drive,arrival:hhmm(match.arrival),starts_at:hhmm(match.start),
        finish_at:hhmm(match.finish),time_left:Math.max(0,match.deadline-when.minute),
        ...
        warnings,source:"Lịch công bố · di chuyển ước tính",score:(badge==="POSSIBLE"?0:10000)+(matches(e,cfg,input.interest)&&input.interest!=="all"?-150:0)+Math.max(0,match.deadline-when.minute)*-0.2+drive.minutes*1.4+(cfg.order||0)
      });
    }
